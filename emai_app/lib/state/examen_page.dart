                ...
                SectionCard(
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        onPressed:
                            scan.loading ? null : () => _pickFromGallery(scan),
                        icon: const Icon(Icons.photo_library),
                        label: const Text('Galería'),
                      ),
                      if (scan.lastImage != null)
                        Text(
                          'Imagen lista: ${scan.lastImage!.name}',
                  ...
                  child: Column(
                    ...
                    children: [
                      if (scan.loading) ...[
                        ...
                        Text(
                          'Precisión: ${(scan.accuracy * 100).toStringAsFixed(0)}%',
                        ),
                      ...
                      _GradePicker(
                        onChanged: (g) {
                          setState(() {
                            _selectedGrade = g;
                            _selectedEvalId =
                                null; // limpiar eval al cambiar grado
                          });
                        },
                      ),
                      const SizedBox(height: 12),
...
class _StudentPicker extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final db = context.read<ScanState>().db;
...
class _EvaluationPicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final db = context.read<ScanState>().db;
    return StreamBuilder<List<Examen>>(
      stream: db.watchEvaluations(grade: grade),
      builder: (context, snap) {
        final items = snap.data ?? const [];
        return DropdownButtonFormField<String>(
          value: selectedId,
          ...
          items:
              items
                  .map(
                    (x) => DropdownMenuItem(
                      value: x.id,
                      child: Text('${x.title} — ${x.grade} — ${_fmt(x.date)}'),
                    ),
                  )
                  .toList(),
          onChanged: onChanged,
...
class _GradePicker extends StatelessWidget {
  final ValueChanged<String?> onChanged;
  const _GradePicker({required this.onChanged});

  @override
  Widget build(BuildContext context) {
    const grados = ['1A', '2A', '3A', '4A', '5A'];
    return DropdownButtonFormField<String>(
...
// Widget para mostrar cada resultado individual
class ResultItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final color = correct ? Colors.green : scheme.error;
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: expectedMsg != null ? Text(expectedMsg!) : null,
      ),
    );
  }
}
